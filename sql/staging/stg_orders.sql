...
WITH
  source AS (
    SELECT
      *
    FROM
      `quintoandar-ecommerce-analysis`.olist_raw.orders
  ),
  cleaned AS (
    SELECT
      order_id,
    ...
    FROM
      source
    WHERE
      1 = 1 AND order_id IS NOT NULL AND customer_id IS NOT NULL AND order_purchase_timestamp <= `CURRENT_TIMESTAMP`()
  ),
  validated AS (
    SELECT
      *
    FROM
      cleaned
    WHERE
      1 = 1 AND (order_approved_at IS NULL OR order_approved_at >= order_purchase_timestamp) AND (order_delivered_carrier_date IS NULL OR
      order_delivered_carrier_date >= COALESCE(order_approved_at, order_purchase_timestamp)) AND (order_delivered_customer_date IS NULL OR
      order_delivered_customer_date >= COALESCE(order_delivered_carrier_date, order_approved_at, order_purchase_timestamp))
  ),
  deduplicated AS (
    SELECT
      * EXCEPT (row_num)
        ...
        SELECT
          *,
          ROW_NUMBER() OVER (PARTITION BY order_id
            ORDER BY order_purchase_timestamp) AS row_num
        FROM
          validated
      )
    WHERE
